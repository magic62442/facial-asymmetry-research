...
def visualize_asymmetry(original_obj, mirrored_registered_obj, output_pdf='asymmetry_heatmap.pdf',
                       ...
                       max_distance=None, cutoff_distance=None, generate_visualization=True):
    """
    可视化面部不对称性热力图
...
def batch_visualize_asymmetry_from_csv(
    ...
    for csv_path in sorted(csv_files):
        ...
        # 从文件名提取max_distance（xiahedian用offset，其他用offset*2）
        multiplier = 1 if 'xiahedian' in input_dir else 2
        file_max_distance = extract_max_from_filename(csv_filename, multiplier=multiplier)
        if file_max_distance is not None:
            print(f"  从文件名提取的colorbar最大值: {file_max_distance} mm")
        else:
            file_max_distance = max_distance  # 回退到传入的默认值
...
def save_summary_statistics(results, output_csv, method_name='MeshMonk'):
    ...
    with open(output_csv, 'w', newline='', encoding='utf-8') as f:
        ...
        # 写入每个文件的数据
        for result in valid_results:
            # 获取文件名（优先使用原始文件名/CSV文件名，其次回退到输出文件）
            if result.get('original_file'):
                filename = result['original_file']
            elif result.get('csv_file'):
...
if __name__ == "__main__":
    ...
    # 处理xiahedian目录 - ICP方法
    results_xiahedian_icp = batch_visualize_asymmetry(
        input_dir='xiahedian',
        colormap='jet',
        region_labels_path='region_labels.txt',
        max_distance=None,  # None表示自动计算每个文件的最大值
    ...
    # 保存ICP汇总统计
    save_summary_statistics(
        results_xiahedian_icp,
        output_csv='xiahedian/icp_result/icp_summary_statistics.csv',
        method_name='ICP'
    )
    # ========== MeshMonk CSV批量处理 ==========
    print("\n" + "=" * 80)
    ...
    # 处理xiahedian目录 - MeshMonk CSV方法
    results_xiahedian_csv = batch_visualize_asymmetry_from_csv(
        input_dir='xiahedian',
        csv_subdir='meshmonk',
        colormap='jet',
        region_labels_path='region_labels.txt',
    ...
    # 保存MeshMonk汇总统计
    save_summary_statistics(
        results_xiahedian_csv,
        output_csv='xiahedian/meshmonk/meshmonk_summary_statistics.csv',
        method_name='MeshMonk'
    )
    print("\n" + "=" * 80)
    print("所有批处理完成!")
